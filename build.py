...
def fetch_contributions(username):
    """Fetch GitHub contribution data from jogruber API. Returns dict or None on failure."""
    url = 'https://github-contributions-api.jogruber.de/v4/' + username + '?y=last'
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'edgewood-park-build/1.0'})
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode('utf-8'))
        total = data.get('total', {}).get('lastYear', 0)
        contributions = data.get('contributions', [])
        print(f'  contributions: {total} total, {len(contributions)} days')
...
def generate_js(hero, hiw, projects, experience, testimonials, contact, contributions):
    ...
    lines = [
        ...
        '',
        'var contributionsData = ' + json.dumps(contributions, indent=2, ensure_ascii=False) + ';',
        '',
    ]
    return '\n'.join(lines)
...
def main():
    ...
    contributions = fetch_contributions('ukogan')

    js = generate_js(hero, hiw, projects, experience, testimonials, contact, contributions)
